    ...
    uint64_t msgSize, uint32_t threadCount)
{
    ...
    msg->setType(DELAY_PERF_MSGTYPE);
    msg->setData(std::make_shared<bytes>(strMsg.begin(), strMsg.end()));
...
void workAsServer(std::string listenIp, uint16_t listenPort, bool disableSsl, uint32_t threadCount)
{
    ...
    wsService->registerMsgHandler(DELAY_PERF_MSGTYPE,
        [&totalRecvDataSize, &lastSecTotalRecvDataSize](
            std::shared_ptr<WsMessage> _msg, std::shared_ptr<WsSession> _session) {
            totalRecvDataSize += _msg->data()->size();
            lastSecTotalRecvDataSize += _msg->data()->size();
